...
**Bug fixed (September 2026, round 4) — legend icons looked crude/indistinguishable compared to the admin editor:** the `minStrokePx`/`minFontPx` floor from the previous round kept POI icons from vanishing entirely, but leaning on a hard floor alone can still look thick/blobby rather than a faithful small copy of the real symbol, and does nothing at all for a FILLED (no-stroke) glyph like the water-drop/medical-cross icons. Bumped `iconBox`/`rowH` up several times chasing this (22px → 30px → 38px, in BOTH `legendRenderer.js`'s `drawLegendCard` and `SectorLocalImageCanvasComponent.vue`'s own duplicate) before landing on the STRUCTURAL fix below.

**Bug fixed (September 2026, round 5) — "Points of interest" rows were a structural problem, not a sizing one:** a POI marker (`DrawingTools.vue`'s `_buildPoiParts`) exports as `[headCircle, tail, iconBg, ...iconParts]` — a big teardrop pin (`headCircle`+`tail`, the marker's real accent color) wrapped around a small white disc (`iconBg`) holding the actual identifying glyph (`iconParts`, e.g. a walking figure, a bold "H", a bicycle, a water drop, a fork+knife). Every previous round scaled the WHOLE group uniformly to fit the legend's `iconBox`, sized off the group's OVERALL bounding box — which is dominated by the big outer pin, so the SAME shrink factor that's harmless for the pin crushes the much-smaller glyph inside it, no matter how large `iconBox` gets. Fixed with a real structural change instead of another size bump: new `decomposePoiMarker()` (`legendRenderer.js`, mirrored as `_decomposePoiMarker()` in `SectorLocalImageCanvasComponent.vue`'s own duplicate) pulls a POI sample apart into its REAL accent color (from `headCircle`'s `fillColor`) and its REAL glyph paths (`iconParts`, identified as the `isHole`-flagged children after the first white `isHole` circle, `iconBg`) — nothing invented. `drawPoiLegendIcon()`/`_drawPoiLegendIcon()` then draws a plain colored circle (no teardrop tail — a legend swatch, not a literal miniature of the on-photo pin) with the real glyph scaled to fill it, using the GLYPH's own bounds for the fit instead of the whole marker's. Returns `null` for anything not shaped like `_buildPoiParts` (every other symbol family — topo/anchor/landmark — keeps the original generic whole-shape render; Landmark markers in particular, e.g. Parking's blue/white square, were never affected by this bug in the first place since they're built differently and don't have a huge outer shape dominating their bounds).

Deliberately drops the pin's teardrop tail rather than reproducing it: scaling the FULL pin (tail included) so the glyph stays legible would make the marker's overall height roughly 2.4× the icon box, overflowing into neighboring legend rows. The color and glyph — the actually informative parts — are the real, unmodified drawn elements; only the decorative pointer shape (meaningless in a legend, which isn't pointing at anything) is simplified to a circle.

**Bug fixed (September 2026, round 6) — the legend position/size toolbar picker "silently did nothing" whenever another sector/route/pitch sharing the same photo already had a real position saved:** every combined-legend resolver (`legendRenderer.js`'s `drawCombinedLegend`, `canvasOverlaysMixin.js`'s `computeEditorLegend`, `SectorLocalImageCanvasComponent.vue`'s `_resolveLegendMeta`) picked whichever sibling document's `legendPosition`/`legendScale` it happened to see FIRST (fixed order in the first two, `this.layouts` array order in the third) — since `DrawingTools.vue`'s `rebuildLegend()` writes `legendPosition`/`legendScale` into every item's own saved json on every rebuild regardless of whether the admin actually touched the picker for THAT item, almost every sibling on a shared photo "has a real position" by the time a second sector/route/pitch gets edited. That meant changing the toolbar's position/scale picker while editing anything other than whichever item happened to win the fixed-order tiebreak had **zero visible effect** — not on the editor's own live preview, not on the baked composite, not on the public page — even though the change was correctly saved into that item's own document. Fixed by adding `legendUpdatedAt` (`DrawingTools.vue`'s `rebuildLegend`, a plain `Date.now()`, only advanced on an EXPLICIT toolbar call — i.e. `position`/`scale` actually passed in, not the internal "just refresh with whatever was already stored" calls after every add/erase/delete) alongside `legendPosition`/`legendScale`, threaded through `findLegendMeta()`'s return value as `updatedAt`, and changing all three resolvers to prefer whichever document's `updatedAt` is highest instead of a fixed first-found — so the admin's most recent choice on ANY item sharing the photo always wins, going forward. Comparisons use `>` (not `>=`), so legacy documents saved before this fix (all `updatedAt` 0) keep resolving to the exact same first-found fallback as before — this only changes behavior once a NEW explicit choice is made after the fix ships.

**Reverted (September 2026, round 7) — round 5's decompose-into-a-plain-circle approach was itself wrong, not just imperfect; every POI legend row must be an exact small copy of the real drawn pin, teardrop tail included:** two independent problems, found by an offline visual test (`drawItem`/`drawLegendCard` run standalone under `node-canvas` against synthetic marker JSON built from `_buildPoiParts`'s own formulas, comparing "real on-photo size" vs. "legend icon" renders side by side):
1. **"bed" rows silently rendered as a blank white circle — no "H" at all.** `decomposePoiMarker()`'s `glyphParts` for `bed` is a single `PointText` (`_drawPoiBedIcon`'s "H" letter, no `Path` sibling). `drawPoiLegendIcon()` measured those parts with `rawNodeBounds()`, which only walks `Group`/`CompoundPath`/`Path` nodes — it has no `PointText` case at all — so `bounds` came back `null` and the whole `if (bounds) {...}` guard skipped drawing the glyph entirely, every time, for every bed marker.
2. Even where it didn't outright drop the glyph, replacing the real teardrop pin with a plain circle was a genuine fidelity regression, not merely a stylistic simplification: a legend row is supposed to teach the reader what the actual on-photo symbol looks like, and a circle+glyph doesn't look like the pin they'll actually see planted on the photo.
Fixed by deleting `decomposePoiMarker()`/`drawPoiLegendIcon()` (`legendRenderer.js`) and their mirrors `_decomposePoiMarker()`/`_drawPoiLegendIcon()` (`SectorLocalImageCanvasComponent.vue`) outright — POI markers now go through the exact same generic "clone the real sample, fit `iconBox`/`maxDim` off `rawNodeBounds`, apply `minStrokePx`/`minFontPx`" path every other symbol family already used, with `iconBox` staying at round 4's 48px (already proven wide enough by the same visual test — the water-drop/medical-cross/bed/bike glyphs, the hardest cases, all render clearly at that size once the WHOLE pin, tail included, is the thing being fit and drawn instead of a decomposed substitute). No more special-casing by symbol family anywhere in `drawLegendCard`: every row is now, in a literal geometric sense, a scaled-down copy of the exact JSON drawn on the photo — nothing invented, nothing dropped, nothing decomposed.

**Bug fixed (September 2026, round 8) — Parking's legend row (and any other shape leaning on the same "hidden strokeWidth" trick) rendered with a phantom red/oversized border, and every "no-strokeColor" shape's REAL geometry was wrong wherever `drawItem` is used:** two compounding bugs, both found by rendering `paperJsonRenderer.js`'s `drawItem` standalone against REAL saved sector JSON pulled from the database (not synthetic test data — the offline synthetic tests from round 5-7 happened to omit the exact properties that trigger these):
1. **Phantom stroke on shapes with no `strokeColor`.** Several `DrawingTools.vue` builders (`_buildParkingParts`' `parking-body`, `_buildPoiParts`/`_buildSummitParts`/`_buildTentParts`' headCircle/body) set `strokeWidth` on a path with NO `strokeColor`, purely to invisibly store the symbol's "size" for later resize — real Paper.js only draws a stroke when `strokeColor` is set, so this relies on staying invisible. `drawItem` always called `ctx.stroke()` regardless, falling back to a hardcoded red (`#cc2222`) whenever `strokeColor` was absent — an unwanted red ring around every such shape, most visible on Parking's square. Fixed: only stroke when there's an ACTUAL color to stroke with (an explicit override, or the item's own real `strokeColor`) — matching Paper.js's own "no strokeColor means no stroke" rule. Same fix applied to `rawNodeBounds()` (`legendRenderer.js` and `SectorLocalImageCanvasComponent.vue`'s mirror), which was ALSO padding bounding boxes by that same phantom stroke's width, inflating a symbol's computed size and shrinking it more than it should in the legend's `iconBox` fit.
2. **Wrong fallback stroke width/font size.** Paper.js's own class defaults are `strokeWidth: 1` and `fontSize: 12` — `exportJSON()` omits any property left at its default, so a deliberately thin accent line (e.g. Parking's white inset border, `strokeWidth: Math.max(1, R*0.1)`, which clamps to exactly `1` for a typical marker size) round-trips with NO `strokeWidth` field at all. `drawItem` assumed `data.strokeWidth || 3` / `data.fontSize || 20` — a 3x-too-thick line once it round-tripped through the missing-property fallback, which at legend-icon scale (~3x further) swelled a subtle inset accent line into a band thick enough to swallow most of the surrounding blue fill, reading as a broken/wrong-colored outline even after fix #1. Fixed the fallbacks to `1`/`12` to match Paper.js's real defaults, in both `paperJsonRenderer.js` and `CanvasJsonDataShowComponent.vue`'s own duplicate `drawItem`.

**Bug fixed (September 2026, round 9) — on-photo signs stayed pinned to the photo's native-pixel proportions with NO screen-size correction, while the legend (correctly) did, so the two drifted wildly out of proportion with each other on any screen narrower than the photo's own native resolution — reported as "signs and legend sizing in viewer... totally not correct":** confirmed by rendering a real two-sector photo (960×1280 native, several bolt/anchor/POI/parking markers) through the full `SectorLocalImageCanvasComponent.vue` pipeline at a simulated narrow (400px) CSS display width — the on-photo pins rendered as barely-visible ~20px specks while the combined legend (13 rows across 4 categories, each correctly boosted to stay legible per `_cssScale()`) grew to 2220px tall, nearly double the photo's own native height, completely dwarfing and overflowing past it. Two fixes:
1. **Markers now get the same legibility boost the legend already had**, via new `_collectMarkerGroups()`/`_drawMarkersBoosted()`: every top-level topo-symbol/anchor/landmark/POI marker Group (see `MARKER_FLAGS`) is drawn individually, scaled up by `_cssScale()` (the SAME ratio `drawLegends()` uses, extracted into a shared method so the two can never drift apart from each other again) around its own real anchor point — bottom-tip for pin/tent-shaped markers (`BOTTOM_ANCHORED_FLAGS`, keeping the marked point exactly in place, the same convention `DrawingTools.vue`'s own `resizePoi`/`resizeSummit`/`resizeTent` use), center for ring/flat-shaped ones — capped to `[1, 3]` so a marker never shrinks below its own authored size (a photo shown bigger than native needs no help) nor balloons past 3x even on a very narrow screen. The flat single-pass content draw in `draw()` now skips every `MARKER_FLAGS` group (drawn separately, boosted, right after) alongside the pre-existing skip list.
2. **The combined legend can no longer grow bigger than the photo it sits on.** `drawLegends()`'s sizing math was extracted into a `measure(scale)` closure so it can be re-run at a smaller scale: if the initially-measured card would exceed 90% of the photo's own width or height (an increasingly likely outcome as more sectors sharing one photo each contribute their own distinct symbol types to the combined legend), `scale` is shrunk by the exact overflow ratio and re-measured once — trading a bit of legibility only in this "very busy legend on a small/narrow photo" case, instead of letting the card grow without limit.

**Bug fixed (September 2026, round 10) — round 9's own fix reintroduced the exact same class of "signs vs. legend don't match" mismatch, just from the opposite direction — reported as "sizing again is [wrong]":** round 9 boosted on-photo markers by the raw `_cssScale()` ratio, and separately boosted (then possibly capped down) the legend by that same raw ratio via its own "never bigger than the photo" overflow guard — consistent ONLY as long as that overflow cap never actually triggered. The moment a photo's combined legend was busy enough to NEED shrinking (precisely the scenario round 9's overflow fix exists for — confirmed with the same real two-sector/13-row photo used to find round 9's bug, at a moderate 400px simulated CSS width: legend overflow ratio 1.93, shrinking its effective scale from 2.4 down to 1.245), markers kept using the full uncapped 2.4x boost while the legend's own icons only got 1.245x — markers rendered noticeably BIGGER than the legend rows meant to represent them, a fresh mismatch in the opposite direction from round 9's original bug (where markers had been too SMALL relative to the legend). Fixed by making `_legendLayout()` (renamed from inlining directly in `drawLegends()`) a pure measurement function — entries/meta/layout/overflow-capped `scale`, no drawing — called ONCE per `draw()`, with its resulting `scale` shared by BOTH `_drawMarkersBoosted()` (as `rawScale`, still capped to `[1, 3]` before use) and the actual legend render (`drawLegends()` now takes the precomputed layout object instead of recomputing its own scale). The two can no longer be computed independently of each other, so they can no longer drift apart regardless of how busy any given photo's combined legend gets.

**Bug fixed (September 2026, round 11) — the admin editor's OWN live legend preview (`canvasOverlaysMixin.js`'s `computeEditorLegend`, shared by all 5 combined-legend editor pages) could render dramatically oversized, dwarfing the photo — reported with a screenshot: the editor's legend nearly a full screen tall next to a normal-looking public-page legend for the SAME sector, both `legendScale: 1` in the saved data (checked directly in the DB), ruling out a toolbar Size-picker cause:** unlike `SectorLocalImageCanvasComponent.vue`'s public viewer (fixed in round 9-10 with a `_cssScale()`/native-resolution correction AND a "never bigger than the photo" overflow cap), this live preview had NEVER had any size ceiling at all — it trusted `scale = Math.max(0.3, resolvedMeta.scale || 1)` completely, then converted straight to screen pixels via `projectToView`. Since that project-space-to-CSS-pixel relationship depends on the Paper.js view staying correctly fitted to its container (see `CanvasManager.vue`'s `loadBackgroundRaster`, which only (re-)fits `view.viewSize` to the container's width once, on image load), anything that leaves the container's actual on-screen width different from what the view was fitted to at load time — a later layout reflow, panel resize, etc., with nothing currently re-fitting the view to match — inflates the preview with no limit. Fixed the same way the public viewer already was: measure the card once, and if it would exceed 90% of the photo's own `bgBounds` width/height (already available in this function, same project-space units the measured card size is in), shrink `scale` by the overflow ratio and re-measure once before the real draw — robust to whatever the underlying cause of the inflation is, since it bounds the OUTPUT directly rather than trying to correct the input. Fixes all 5 editor pages that share this mixin (sector-local-image, spot-rock-image, route, route-drawings, MTP-pitch) at once.

**Changed (September 2026, round 12) — the Legend Position picker (`ToolbarComponent.vue`) is now a visual 3x3 anchor-point grid instead of a plain text `<select>`:** each of the 8 real corner/edge positions sits at its own matching spot in the grid (top-left cell = "top-left", etc.), so picking one is a single recognizable click instead of reading through 9 text option strings ("legend position and size selectors match friendly"). The unused center cell doubles as "hidden" (an eye-slash icon) — the semantic odd one out anyway, since it means "don't show it" rather than pinning to an edge. New `legendPositionGrid` computed property drives the `v-for`; emits the exact same `legend-position-change` event with the exact same position-string values as before (`'top-left'`, `'hidden'`, etc.), so nothing downstream (`EditorComponent.vue`'s `handleLegendPositionChange`, `DrawingTools.vue`'s `rebuildLegend`) needed to change. The Legend Size picker is untouched (still a `<select>`, renamed its own CSS class from the now-inaccurate `legend-position-select` to `legend-scale-select` since it no longer shares a selector with the position picker).

**Changed (September 2026, round 13) — the Legend Size picker is now a row of pill buttons instead of a plain `<select>`, and the whole Legend Position/Size group hides itself when there's nothing for it to affect:** two related fixes, both reported directly ("i cant select size it blinking and i cant select thet size whot i whont", "if i dont have a sign item dont show position and size selection"):
1. **Size `<select>` replaced with `legendScaleOptions` buttons.** The exact same 7 fixed sizes (50%-250%), same emitted event/values, just as `.legend-scale-cell` buttons in a `.legend-scale-grid` (styled to match round 12's position grid) instead of `<option>`s. A native `<select>`'s open dropdown list is background-DOM-patch-sensitive in a way a plain button's single click event isn't — this toolbar's legend preview recomputes on every animation frame (`canvasOverlaysMixin.js`'s `_overlaysSyncLoop`), and whatever exact interaction between that constant background work and the open listbox was happening, sidestepping the native dropdown entirely resolves it regardless of the precise mechanism.
2. **New `hasLegendSymbols` prop chain.** `canvasOverlaysMixin.js`'s `computeEditorLegend()` already computes `entries.length > 0` (does ANY sibling sharing this photo have a symbol at all) to decide whether to draw the live preview — now also stored as reactive `hasLegendSymbols`, passed by all 5 host pages to `<Editor :has_legend_symbols>`, forwarded to `<ToolbarComponent :has-legend-symbols>`, which wraps the entire Legend Position/Size tool-group (plus its own separating dividers) in `v-if`. A brand-new sector/route/pitch with nothing drawn on it yet — or one with drawn content but no siblings with any either — no longer shows two controls that visibly do nothing. Defaults `true` on both the prop and the mixin's own initial data, so a host that hasn't wired this up (there currently isn't one, but the prop is optional) keeps the old always-visible behavior.

**Changed (September 2026, round 14) — both pickers refined again per direct user feedback ("make selections match more friendly"), picking the "compass + stepper" option out of a set of previewed alternatives:**
1. **Position grid cells now show a real directional arrow** (↖ ↑ ↗ / ← ✕ → / ↙ ↓ ↘) instead of a plain dot/eye-slash icon, and grew from 18px to 22px — `legendPositionGrid`'s entries each gained an `arrow` field matching their own grid slot's direction; the center "hidden" cell keeps its ✕.
2. **Size buttons replaced with a "− 100% +" stepper** (`legend-scale-stepper`): round 13's 7-button row fixed the underlying `<select>` bug but, on reflection, read as busier and wider than this toolbar column needs for what's really just a "smaller/bigger" control. New `legendScaleIndex` computed (position of the current `legendScale` within `legendScaleOptions`, falling back to 100%'s index for a value that isn't one of the 7 stops) drives a new `stepLegendScale(delta)` method and the +/- buttons' own disabled-at-the-ends state. Two new i18n keys (`legend_scale_decrease`/`legend_scale_increase`, en+ka) label the +/- buttons for screen readers since they no longer have their own visible text beyond the glyph.

**Bug fixed (September 2026, round 15) — a SINGLE undo click could wipe an item's entire drawing, including signs saved in a PREVIOUS session, reported directly ("if i have signs drowing and legends and undo buttom it del evrithink"):** root-caused with an offline reproduction of the actual undo algorithm against the real Paper.js library (`CanvasManager.vue`'s `_getDrawingJson`/`saveCanvasData`/`undoLastAction`, copied verbatim into a standalone Node script) — confirmed the core push/pop mechanism itself is sound (interleaving legend position/scale changes with symbol placement didn't break it), then found the actual bug: `_initHistory()` — which seeds `history` with "whatever's on screen right now" as the undo floor — only ever ran ONCE, at the Editor's initial mount. Every host page mounts its `<Editor>` BEFORE the admin has selected any sector/route/pitch (`jsonProp` starts `null`), so that one-time call captured a BLANK canvas as the permanent floor. Selecting an item with its OWN pre-existing signs (loaded via the `jsonProp` prop-change path, or the `image` prop changing to a different photo entirely) then loads real content on top of that stale blank baseline WITHOUT ever moving the floor — so drawing even one new sign and pressing undo ONCE could unwind straight past every sign already saved on that item, all the way back to the original blank mount state. Fixed by calling `_initHistory()` again after both re-load paths finish (`_reimportStrokes()`, covering the initial mount AND the `image`-watcher path; the `jsonProp` watcher's own handler, covering same-photo item switches like several sectors sharing one image) — each guarded by the SAME echo-check (`newVal === this._lastDrawingJson`) that already protects against reimporting a document's own save round-trip, so normal editing (draw → save → echo back through props) never touches this at all, only a genuine switch to a DIFFERENT document's content does.

**Bug fixed (September 2026, round 16) — round 15's own fix broke layer ordering, hiding an item's OWN drawing behind a sibling's reference overlay after any sector/item switch — reported as "zooming buttom is not working and dont show drowing... all drowing dont working saim [but] sectors have one bugs [and] enother canvass enothers [have different ones]":** the "different symptom per sector/canvas" was the tell — this depends on how many `related-N` overlay layers a given item happens to have, which varies per item/photo. Root-caused with an offline Paper.js reproduction of the actual layer stack (`layer_order_check.js`): `importJsonData()`/`importRelatedJsons()` correctly position every `related-N` layer BELOW `main` via `_repositionRelatedLayersBelow()` — but round 15's new `_initHistory()` call runs immediately after, and `_initHistory()` → `_getDrawingJson()` temporarily REMOVES every `related-N` layer (so a sibling's overlay never gets baked into THIS item's own history/save snapshots) and restores them with a plain `project.addLayer()`, which appends to the END of the layers array — i.e. drawn ON TOP of `main`, undoing the correct ordering that had just been established one line earlier. Confirmed via the reproduction: layer order went `[background, related-1, main]` (correct) → `[background, main, related-1]` (broken) across exactly that sequence. A sibling's overlay rendering on top of — fully hiding — the actively-edited item's own drawing reads exactly like "drawing/zoom stopped working" (the canvas visibly does nothing when you interact with it, because what you're looking at is the wrong layer). Fixed by calling `_repositionRelatedLayersBelow()` again immediately after each of round 15's new `_initHistory()` calls (both in `_reimportStrokes()` and the `jsonProp` watcher) — cheap (just a sort + `insertBelow` per related layer) and idempotent, so it's safe to call defensively every time `_initHistory()` runs from these two spots.

**Changed (September 2026, round 17) — checking a layer's own "select for grouping" checkbox now actually shows it selected on the canvas, and drawing is blocked entirely until a sector/spot-rock item is chosen (matching how the route/pitch editors already worked):**
1. **`toggleLayerSelection()`** (`EditorComponent.vue`) previously only tracked `selectedLayerIds` (the id array feeding the "Group N" button) with no effect on the canvas at all — reported directly ("if i select some element need show it in paper"). Now also flips the real Paper.js item's own `.selected` (the same cyan outline the Move/Select tool draws) alongside the id-array bookkeeping, and `createGroupFromSelection()` clears it again on the items once they're re-parented into the new group.
2. **`sectorLocaleImageEditorComponent.vue`/`spotRockImageEditorComponent.vue`'s `<Editor>`** was gated only on `imageUrl` (the shared photo having loaded) — reported as "if i dont select some entity (sector/route/mtp) make imposeble to make some drowing... for route and mtp/mtp-pitch it work normal for local-sectors and spot image is not": both `sectorRouteDrawingsEditorComponent.vue` (route/pitch) and `CanvasRouteEditorComponent.vue` already hide their own `<Editor>` entirely behind `v-if="selectedRouteId || mtp_pitch_mode"` / `v-if="sector_id_prop != ''"`, so drawing before selecting one is simply impossible there — the two sector-local-image/spot-rock pages never had the equivalent gate on `selectedSectorId`. Since the photo loads independently of which sector's layout is being edited, the canvas was fully drawable with NO sector chosen: `handleCanvasData` still populates `canvasData` from that drawing, `saveChanges()`'s `!selectedSectorId` guard blocks the actual save with an alert, and picking a sector afterward overwrites `canvasData` with THAT sector's own saved json — silently discarding everything just drawn. Fixed by adding the matching `v-if="imageUrl && (selectedSectorId || extra_drawing_mode)"` condition (Extra Drawing mode is independent of any one sector, so it stays a valid alternative gate) plus a distinct "select a sector first" placeholder (reusing the existing `select_sector_first_alert` i18n key) separate from the "loading image" one.
