...
1. **`show()` has no ban check.** `list()` explicitly excludes banned users; `show($user_id)` does not — a banned climber's profile page remains fully reachable by direct URL.
