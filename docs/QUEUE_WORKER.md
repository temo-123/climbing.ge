...
**What was actually broken:** the config file had a typo — `stdout_logfile=/var/www/http/storage/logs/worker.log` (**http**, not **html**). Since `/var/www/http/` doesn't exist, `supervisord` refused to start at all — not just the worker, the whole supervisor daemon — and `systemd` had been silently crash-looping it every ~50 seconds for a very long time (restart counter was over 2000). The documented fix is correcting the path to `/var/www/html/`.

> **Verify this on your actual deployment target before trusting it's fixed.** A September 2026 documentation audit found `/etc/supervisor/conf.d/laravel-worker.conf` on the environment it was run in *still* had the `/var/www/http/` typo, no `queue:work`/`supervisord` process running, and no `worker.log` file — i.e. this fix may never have actually been applied outside whichever environment it was first diagnosed on, or a redeploy reset the conf file. Don't assume "documented as fixed" means "fixed here" — check with `grep http /etc/supervisor/conf.d/laravel-worker.conf` (should return nothing) and `sudo supervisorctl status` (should show `laravel-worker` as `RUNNING`) on the box you actually care about.
...
`laravel/horizon` is already a Composer dependency (`composer.json`), with `app/Providers/HorizonServiceProvider.php` and `config/horizon.php` present — so `php artisan horizon` is available today, it's just not wired into supervisor. Horizon is a dashboard + supervisor-like process manager built specifically for Laravel queues (metrics, retry UI, per-queue throughput). It's not currently configured to run persistently here (no supervisor entry for it), but if the manual `queue:work` approach ever becomes hard to observe, wiring Horizon in behind its own supervisor program (same pattern as `laravel-worker` above, just running `php artisan horizon` instead) is the natural upgrade path. Horizon's dashboard is served at `/horizon` on whichever domain the app boots on — gate it in `app/Providers/HorizonServiceProvider.php`'s `gate()` method before ever exposing it beyond local.
