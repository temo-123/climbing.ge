<template>
    <div class="gallery-manager">
        <div class="d-flex align-items-center justify-content-between mb-3">
            <h5 class="mb-0">{{ title_prop || $t('admin.gallery_manager.gallery_images_default_title') }}</h5>
        </div>

        <!-- Existing images from DB -->
        <div v-if="old_images.length > 0" class="mb-4">
            <p class="text-muted small mb-2">{{ $t('admin.gallery_manager.saved_images_count_label', { count: old_images.length }) }}</p>
            <div class="row g-2">
        ...
        <div v-else class="text-muted small text-center py-3">
            {{ $t('admin.gallery_manager.no_new_images_added_yet') }}
        </div>
...
<script>
    ...
    export default {
        ...
        props: {
            ...
            get_images_route_prop: { type: String, default: '' },
            item_id_prop: { type: [String, Number], default: null },
        ...
        methods: {
            ...
            async handleExternalDrop(event) {
                ...
                if (this.draggedIndex !== null) return; // internal reorder drop
            ...
            async compressIfNeeded(file) {
                ...
                try {
                    const compressed = await this.compressImage(file, maxBytes);
                    return compressed;
                } finally {
            ...
            get_editing_images() {
                const id = this.item_id_prop !== null ? this.item_id_prop : this.$route.params.id;
                if (!this.get_images_route_prop || !id) return;
                axios
                    .get(this.get_images_route_prop + id)
                    .then(response => { this.old_images = response.data; })
                    .catch(() => {});
            },

            del_image_from_db(image_id) {
                if (!confirm(this.$t('admin.gallery_manager.confirm_delete_image'))) return;
                axios
                    .delete(this.image_del_route_prop + image_id)
                    .then(() => { this.get_editing_images(); })
