<template>
    <div class="tabs">
        ...
        <div class="row" v-show="!is_loading">
            <div class="form-group">
                <button type="button" class="btn btn-primary" @click="edit_member()">{{ $t('common.save') }}</button>
            </div>
        </div>
        ...
        <div class="row" v-show="!is_loading">
            <div class="col-md-12">
                <span class="badge badge-success mb-3" v-if="data.user_id">{{ $t('admin.partner_organizations.linked_user_col') }}</span>
...
export default {
    ...
    methods: {
        ...
        get_editing_data() {
            axios.get('/set_partner_organization_member/get_editing_member_data/' + this.$route.params.id)
                .then(response => {
        ...
        edit_member() {
            ...
            axios.post('/set_partner_organization_member/edit_member/' + this.$route.params.id, { data: this.data })
                .then(response => {
                    this.go_back(true)
                })
                .catch(err => {
