...
<script>
export default {
    name: 'TrainingPlanEditComponent',
    data() {
        return {
            tab_num: 1,
            is_loading: false,
    ...
    methods: {
        ...
        load() {
            ...
            axios
            .get('/set_training_plan/get_plan_data/' + this.$route.params.id)
            .then(response => {
                const data = response.data;
                this.form.name = data.name;
                this.form.emoji = data.emoji;
                this.form.level = data.level;
                this.form.tagline = data.tagline;
                this.form.description = data.description;
                this.form.coach_note = data.coach_note;
                this.form.days_per_week = data.days_per_week;
                this.form.weeks = data.weeks;
                this.form.is_published = data.is_published ? 1 : 0;
                this.form.sessions = (data.sessions || []).map(session => ({
                    day_index: session.day_index,
                    day_label: session.day_label,
                    training_ids: (session.plan_session_trainings || []).map(pst => pst.training_id),
                }));
                ...
                if (kaTranslation) {
                    this.form.translations.ka = {
                        ...
                        tagline: kaTranslation.tagline,
                        description: kaTranslation.description,
                        coach_note: kaTranslation.coach_note,
                    };
                }
            })
            .catch(error => console.log(error))
        ...
        save() {
            ...
            axios
            .post('/set_training_plan/update_plan/' + this.$route.params.id, this.form)
            .then(() => {
