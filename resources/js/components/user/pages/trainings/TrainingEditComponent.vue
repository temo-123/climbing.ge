...
export default {
    ...
    methods: {
        load() {
            ...
            .then(response => {
                ...
                if (data.product_id) {
                    ...
                    const subcategory = data.product && data.product.product_subcategory;
                    if (subcategory) {
                        this.selectedCategoryId = subcategory.category_id;
                        axios.get('/get_product/get_product_category/get_subcategory/get_subcategories_for_category/' + subcategory.category_id)
                            .then(response => {
                                this.subcategories = response.data || [];
                                this.selectedSubcategoryId = subcategory.id;
                                return axios.get('/set_training/search_products', { params: { subcategory_id: subcategory.id } });
                            })
                            .then(response => {
        ...
        save() {
            ...
            axios
            .post('/set_training/update_training/' + this.$route.params.id, this.buildFormData())
            .then(() => {
