        ...
        <div class="item">
            <div class="product_image">
                ...
                <router-link :to="'product/'+product.global_product.url_title">
                    <div class="item-img">
                        <site-img v-if="product.product_images.length" :src="'/public/images/product_img/'+get_product_image()" :alt="product.us_product.title" />
                        <site-img v-else :src="'/public/images/site_img/shop_demo.jpg'" :alt="product.us_product.title" />
                    </div>
                </router-link>
                <div class="next_image" v-if="image_num < (this.image_length - 1)">
                    <a @click="next_product_image()">></a>
                </div>
                <div class="product_quick_view" @click="product_quick_view(product.global_product.id)">Quick view</div>
            </div>
            <div class="item-pnl product-image">
                <div class="discount-percent-badge discount-badge-fourty" v-if="product.discount">-{{ product.discount }}%</div>
                <div class="pnl-wrapper">
                    <div class="pnl-description">

                        <router-link :to="'product/'+product.global_product.url_title">
                            <span class="pnl-label">
                                <h2>{{ product.us_product.title }}</h2>
                            </span>
                        </router-link>

                        <div class="price">
                            <span class="pnl-price price">
                                <span class="pnl-price price" v-if="product.min_price != product.max_price" >{{ product.min_price }} ₾ - {{ product.max_price }} ₾</span>
                                <span class="pnl-price price" v-else-if="product.min_price == product.max_price">{{ product.max_price }} ₾</span>
                            </span>
                        </div>
                    </div>

                    <div class="pnl-favorites">
                        <i class="fa fa-heart-o favorite_icon" @click="favorite_product(product.global_product.id)" ></i>
                    </div>

                    <!-- <div class="pnl-favorites">
                        <i class="fa fa-cart-plus favorite_icon" @click="add_to_cart(product.global_product.id)"></i>
                    </div> -->
                </div>
            </div>
        </div>
