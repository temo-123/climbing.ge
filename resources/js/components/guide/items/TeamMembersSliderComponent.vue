<template>
    <div class="h-recent-work services" id="services" v-if="team_members.length > 0">
        <div class="container">
            ...
            <div class="team-members-slider-container">
                ...
                <div ref="teamSliderWrapper" class="team-slider-wrapper" @mouseenter="pauseAutoSlide" @mouseleave="resumeAutoSlide">
                    <div class="team-slider" :style="{ display: 'flex', width: (team_members.length * (100 / visibleCount)) + '%', transform: 'translateX(' + (-slider_index * 100 / team_members.length) + '%)', transition: 'transform 0.5s ease' }">
                        <div class="team-item" v-for="user in team_members" :key="user.id" :style="{ flex: '0 0 ' + (100 / team_members.length) + '%' }">
                            <div class="team-card" @click="show_user_modal(user.id)">
                                ...
                                <span class="user_status" v-if="user.member_status">{{ user.member_status }}</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="team-slider-dots" v-if="team_members.length > visibleCount">
                <span
                    v-for="dotIndex in (team_members.length - visibleCount + 1)"
                    :key="dotIndex"
                    class="team-dot"
